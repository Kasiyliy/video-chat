...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    peer.on('call', function (call) {
        var acceptsCall = confirm("Сізге бір кісі звондағалы жатыр, қабылдағыңыз келе ме?");
        ...
        if(acceptsCall){
            ...
            call.on('close', function(){
                alert("Видео чат аяқталды");
            });

            // use call.close() to finish a call
        }else{
            console.log("Адам сіздің қоңырауыңызды алмады !");
        }
    });
    ...
    document.getElementById("call").addEventListener("click", function(){
        console.log('Звондап жатырмын: ' + peer_id);
        console.log(peer);
    ...
    requestLocalVideo({
        ...
        error: function(err){
            alert("Сіздің камераңызға кіру мүмкін емес !");
            console.error(err);
        }
    });
}, false);
